---
name: specify
description: Spec out a change as a Linear RFC issue. Grills the user first to sharpen domain language, then publishes a parent Linear issue with a terse human-facing RFC at the top and a comprehensive agent-facing PLAN.md collapsible at the bottom. Use when the user wants to spec out a change, write an RFC, or stress-test an idea against the existing domain model. Trigger on phrases like "spec this out", "write an RFC for X", "create an RFC", "stress-test this idea".
---
...
2. **Build at least two frames** with `use_figma`:
   - One **Closed** / current-state frame: the existing shell (rail, header, sidebar, table, etc.) as you saw it in section 1's screenshots.
